...
list.files(path)
d1 <- read.csv('data/F1_valence_reverse.csv', header=TRUE)
d2 <- read.csv('data/F2_valence_reverse.csv', header=TRUE)
...
#|||| ANGER RATINGS + PLOT|||# # need to figure out how to properly order the variables 
...
colnames(anger_vals) <- c("expression_level", "valence_rating", "SE")
...
anger_vals$valence_rating <- as.numeric(as.character(anger_vals$valence_rating))
...
anger_vals %>%
  ...
  ggplot( aes(x=expression_level, y = valence_rating)) +
  geom_col(fill = "#DF0101" ) +
  geom_errorbar(aes(ymin = valence_rating - SE, ymax = valence_rating + SE), width = 0.1) + 
  xlab("Expression Level") + ylab("Valence Rating") +
  ggtitle("Anger") + 
  theme_minimal() +
...
colnames(sad_vals) <- c("expression_level", "valence_rating", "SE")

## this block is needed to reorganize the order of the x axis variables 
sad_vals$expression_level <- factor(sad_vals$expression_level, levels = c("s.low", "s.med", "s.full"))
levels(sad_vals$expression_level)
sad_vals$valence_rating <- as.numeric(as.character(sad_vals$valence_rating))
...
colnames(happy_vals) <- c("expression_level", "valence_rating", "SE")
...
happy_vals$valence_rating <- as.numeric(as.character(happy_vals$valence_rating))
